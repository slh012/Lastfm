...
function lastfm_autoload($name){
	...
	if(stripos($name, 'Cache') !== false){
		$filename = sprintf("%s/Cache/%s.php", dirname(__FILE__), $name);
	}
	else if(stripos($name, 'Caller') !== false){
		$filename = sprintf("%s/Caller/%s.php", dirname(__FILE__), $name);
	}
	else{
		$filename = sprintf("%s/%s.php", dirname(__FILE__), $name);
	}
